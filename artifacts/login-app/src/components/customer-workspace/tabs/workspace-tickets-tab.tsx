...
const TERMINAL: ReadonlySet<TicketStatus> = new Set(["resolved", "closed"]);
        ...
        <table className="w-full min-w-[44rem] table-fixed border-collapse text-sm">
          <thead className="sticky top-0 z-10 bg-background">
            <tr className="border-b border-border/60 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
              <th className="w-[16%] px-3 py-2.5 text-start">{t("tickets.columns.number")}</th>
              <th className="px-3 py-2.5 text-start">{t("tickets.fields.subject")}</th>
              <th className="w-[14%] px-3 py-2.5 text-start">{t("tickets.columns.status")}</th>
              <th className="w-[12%] px-3 py-2.5 text-start">{t("tickets.columns.priority")}</th>
              <th className="w-[14%] px-3 py-2.5 text-start">{t("tickets.columns.sla")}</th>
              <th className="w-[16%] px-3 py-2.5 text-start">{t("tickets.columns.updated")}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((ticket) => {
              // Terminal tickets: no active SLA countdown in Customer 360.
              const slaState = TERMINAL.has(ticket.status)
                ? "none"
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2.5 text-start">
                    <TicketSlaBadge state={slaState} label={t(`tickets.sla.${slaState}`)} />
                  </td>
