...
import { Download, Expand, FileSpreadsheet, FileText, X } from "lucide-react";
    ...
    <>
      <div className="mt-2 flex flex-col gap-2">
        {attachments.map((attachment, index) => {
          const key = `${attachment.url ?? attachment.name ?? "attachment"}-${index}`;
          if (isImageAttachment(attachment) && attachment.url) {
            return (
              <div key={key} className="overflow-hidden rounded-xl ring-1 ring-[var(--ad-border-subtle)]">
                <button
                  type="button"
                  className="group relative block w-full"
                  onClick={() => setFullscreenUrl(attachment.url)}
          ...
          return (
            <div
              key={key}
              className="flex items-center justify-between gap-3 rounded-xl bg-[var(--ad-surface-2)] px-3 py-2 ring-1 ring-[var(--ad-border-subtle)]"
            >
