...
import { InboxQueueChips } from "@/components/omnichannel/workspace-v2/inbox-queue-chips";
...
export const InboxColumn = memo(function InboxColumn({
  ...
  const showChips = Boolean(activeNav && onNavChange && navCounts && navLabels && navAriaLabel);
  ...
  const totalForHeader =
    showChips && navCounts && activeNav ? (navCounts[activeNav] ?? conversations.length) : conversations.length;
  ...
  return (
    <>
      ...
      <section
        ...
        className="ws-inbox-column overflow-hidden bg-[var(--ws-surface)]"
        aria-label={title}
      >
        <header className="flex shrink-0 items-center gap-2 border-b border-[var(--ws-border-subtle)] px-3 py-2.5">
          <h2 className="text-sm font-semibold tracking-tight text-[var(--ws-text)]">{title}</h2>
          <span className="rounded-full bg-[var(--ws-accent-dim)] px-2 py-0.5 text-[11px] font-semibold tabular-nums text-[var(--ws-accent)]">
            {totalForHeader}
          </span>
          <div className="ms-auto flex items-center gap-1">
            {onRefresh ? (
              <button
                type="button"
                className="ws-btn ws-btn--ghost p-1.5"
                onClick={onRefresh}
                disabled={refreshing}
