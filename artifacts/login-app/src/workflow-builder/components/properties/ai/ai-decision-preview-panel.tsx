...
type AIDecisionPreviewPanelProps = {
  preview: AIWorkflowPreviewResult | null;
};

export function AIDecisionPreviewPanel({ preview }: AIDecisionPreviewPanelProps) {
    ...
    <div className="space-y-3">
      ...
      {preview ? (
        <div className="space-y-3 rounded-xl border border-border/60 bg-background/50 p-3 text-sm">
          <div>
            <p className="font-medium">{ai("sections.decisionPreview")}</p>
            <p className="text-xs text-muted-foreground">{ai("sections.decisionPreviewHint")}</p>
          </div>
          <dl className="grid gap-2">
            <div className="flex justify-between gap-3">
              <dt className="text-muted-foreground">{ai("preview.mode")}</dt>
              <dd className="font-medium">{String(preview.nodeMetadata?.decisionModeLabel ?? "—")}</dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-muted-foreground">{ai("preview.outcomes")}</dt>
              <dd className="font-medium">{String(preview.nodeMetadata?.outcomeCount ?? 0)}</dd>
            </div>
          ...
          {preview.nodeMetadata?.expectedOutput ? (
            <div className="space-y-1">
              <p className="text-muted-foreground">{ai("preview.expectedOutput")}</p>
