...
import { normalizeSwitchEdgesForDocument } from "../logic/branch-utils";
import { createEdgeId, type BuilderEdge, type BuilderNode, type BuilderNodeType, type BuilderViewport, type WorkflowDocument } from "../types";
...
export function mapFlowToDocument(
  ...
  const builderNodes = nodes.map(mapRecordToBuilderNode).filter(Boolean) as BuilderNode[];
  const builderEdges = normalizeSwitchEdgesForDocument(
    builderNodes,
    edges.map(mapRecordToBuilderEdge),
    createEdgeId,
  );
  return {
    flowId: flow.id,
