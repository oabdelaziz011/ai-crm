...
import type { MessengerCredentialsLoader } from "../adapters/messenger/messenger-canonical-credentials.js";
import { CompanyChannelNotFoundError } from "../errors.js";

export async function resolveMessengerCompanyChannel(
  ...
  if (!legacyVerifyToken) {
    throw new Error(
      "Messenger webhook verify token is not configured. Update company Messenger settings.",
    );
  }
