...
export class BookingValidationService {
  ...
  async validateBooking(params: ValidateBookingParams): Promise<BookingValidationResult> {
    ...
    const [resource, service, capabilities, bookingRules, customer] = await Promise.all([
      ...
      this.loadCustomer(params.customerId, params.companyId),
    ]);

    if (!customer) {
      errors.push("customer_not_found");
    }
  ...
  private async loadCustomer(customerId: string, companyId: string) {
