/**
 * Apply ONLY migration 363 (Email Workspace tab RBAC).
 * Does NOT grant the new permissions to existing roles.
 *
 * Run: node scripts/apply-363-email-workspace-tab-rbac.mjs
 */
...
const migrationName = "363_email_workspace_tab_rbac.sql";
const version = "363";
...
try {
  ...
  const catalog = await client.query(
    `select code from public.permissions
     where code in ('email.templates.view', 'email.routing.view', 'ai.email.manage')
     order by code`,
  );
  const grants = await client.query(
    `select p.code, count(*)::int as role_grants
     ...
     where p.code in ('email.templates.view', 'email.routing.view', 'ai.email.manage')
     group by p.code
     order by p.code`,
  );
