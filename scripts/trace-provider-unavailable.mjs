/**
 * Trace OpenAI responses through tool-call loop (mirrors webhook runtime).
 */
...
import { ProviderUnavailableError } from "../lib/ai-execution-engine/src/errors/runtime-errors.ts";
...
const loopResult = await loop.run({
  ...
  gatewayRequest: {
    ...
    metadata: {
      ...platformRuntime,
      apiKey: platformRuntime.apiKey,
      baseUrl: platformRuntime.baseUrl,
      model: platformRuntime.model,
    },
    tools: llmTools,
  },
...
const report = {
  platformModel: platformRuntime.model,
  ...
  toolExecutions: loopResult.toolExecutions.map((t) => ({
    toolKey: t.toolKey,
    status: t.status,
...
if (!loopResult.response.text.trim()) {
  ...
  console.log("\nSIMULATED EnterpriseAIRuntimeService.execute throw:");
  console.log("file: lib/ai-execution-engine/src/runtime/enterprise-ai-runtime-service.ts");
  console.log("function: execute");
  console.log("line: 400-401");
