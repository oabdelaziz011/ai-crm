-- Sprint C1 — Instagram channel credentials (canonical store, mirrors WhatsApp pattern).

create table if not exists public.company_instagram_settings (
  company_id uuid primary key references public.companies(id) on delete cascade,
  enabled boolean not null default false,
  provider text not null default 'meta_instagram'
    check (provider in ('meta_instagram')),
...
create index if not exists idx_company_instagram_settings_business_account_id
  on public.company_instagram_settings(instagram_business_account_id)
  where instagram_business_account_id <> '';

create index if not exists idx_company_instagram_settings_verify_token_hash
  on public.company_instagram_settings(webhook_verify_token_lookup_hash)
  where webhook_verify_token_lookup_hash <> '';

alter table public.company_instagram_settings enable row level security;
...
create or replace function public.instagram_settings_has_access_token(settings_row public.company_instagram_settings)
returns boolean
language sql
immutable
as $$
...
create or replace function public.instagram_settings_has_webhook_token(settings_row public.company_instagram_settings)
returns boolean
language sql
immutable
as $$
...
create or replace function public.instagram_settings_has_app_secret(settings_row public.company_instagram_settings)
returns boolean
language sql
immutable
as $$
...
create or replace function public.instagram_settings_row_to_public(settings_row public.company_instagram_settings)
returns jsonb
language sql
stable
as $$
  select jsonb_build_object(
    ...
    'access_token', public.whatsapp_mask_secret(
      ...
      public.instagram_settings_has_access_token(settings_row)
    ),
    'page_id', settings_row.page_id,
    'instagram_business_account_id', settings_row.instagram_business_account_id,
    'webhook_verify_token', public.whatsapp_mask_secret(
      settings_row.webhook_verify_token_hint,
      public.instagram_settings_has_webhook_token(settings_row)
    ),
    ...
    'app_secret', public.whatsapp_mask_secret(
      ...
      public.instagram_settings_has_app_secret(settings_row)
    ),
    'has_access_token', public.instagram_settings_has_access_token(settings_row),
    'has_webhook_verify_token', public.instagram_settings_has_webhook_token(settings_row),
    'has_app_secret', public.instagram_settings_has_app_secret(settings_row),
    'updated_at', settings_row.updated_at
  );
$$;

create or replace function public.sync_instagram_channel_references(p_company_id uuid)
returns void
language plpgsql
security definer
...
as $$
declare
  settings_row public.company_instagram_settings;
  channel_row record;
  next_configuration jsonb;
begin
  select * into settings_row
  from public.company_instagram_settings
  ...
  loop
    ...
    if settings_row.company_id is not null
      and nullif(trim(settings_row.instagram_business_account_id), '') is not null then
      next_configuration := jsonb_set(
        next_configuration,
        '{instagramBusinessAccountId}',
        to_jsonb(trim(settings_row.instagram_business_account_id)),
        true
      );
    end if;
    ...
    next_configuration := jsonb_set(
      next_configuration,
      '{credentialsSource}',
      '"company_instagram_settings"'::jsonb,
      true
    );
...
as $$
declare
  row public.company_instagram_settings;
begin
  ...
  select * into row from public.company_instagram_settings where company_id = p_company_id;
  if not found then
  ...
  return public.instagram_settings_row_to_public(row);
end;
$$;

create or replace function public.get_company_instagram_settings_decrypted(p_company_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
...
create or replace function public.upsert_company_instagram_settings(
  p_company_id uuid,
  p_enabled boolean,
  p_provider text,
  p_access_token text,
...
as $$
declare
  existing public.company_instagram_settings;
...
begin
  ...
  if p_provider not in ('meta_instagram') then
    raise exception 'Invalid Instagram provider';
  end if;

  select * into existing from public.company_instagram_settings where company_id = p_company_id;
  ...
  if p_enabled and p_provider = 'meta_instagram' then
    if nullif(trim(p_instagram_business_account_id), '') is null then
      raise exception 'Instagram Business Account ID is required when Instagram is enabled';
    end if;
    if existing.company_id is null and nullif(trim(resolved_access_token), '') is null then
      raise exception 'Access Token is required when Instagram is enabled';
    end if;
  end if;

  insert into public.company_instagram_settings (
    company_id,
    enabled,
    provider,
    access_token,
    ...
    '',
    coalesce(p_page_id, ''),
    coalesce(p_instagram_business_account_id, ''),
    '',
  ...
  perform public.sync_instagram_channel_references(p_company_id);

  return public.get_company_instagram_settings(p_company_id);
end;
$$;

grant execute on function public.sync_instagram_channel_references(uuid) to service_role;
grant execute on function public.get_company_instagram_settings(uuid) to authenticated, service_role;
grant execute on function public.get_company_instagram_settings_decrypted(uuid) to service_role;
grant execute on function public.upsert_company_instagram_settings(
  uuid, boolean, text, text, text, text, text, text, text
