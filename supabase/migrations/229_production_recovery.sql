-- Production schema recovery: idempotent reconstruction of repo migrations 214–227.
-- Safe to run multiple times. DDL only — no data backfills, DROP, DELETE, or TRUNCATE.
-- For production DBs where migration 214 diverged and migrations 215–227 were never applied.

-- =============================================================================
-- 214 — ai_employee_ticket_management_sprint6_9_1
-- =============================================================================
...
begin
  if not exists (
    ...
    where n.nspname = 'public' and c.relname = 'support_tickets' and t.tgname = 'support_tickets_updated_at' and not t.tgisinternal
  ) then
...
begin
  if not exists (
    ...
    join pg_class c on c.oid = t.tgrelid
    join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'support_tickets' and t.tgname = 'support_tickets_audit' and not t.tgisinternal
  ) then
    create trigger support_tickets_audit after insert or update or delete on public.support_tickets for each row execute function public.write_support_ticket_audit_log();
  end if;
end $$;
...
begin
  if not exists (
    ...
    join pg_class c on c.oid = t.tgrelid
    join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'support_ticket_comments' and t.tgname = 'support_ticket_comments_audit' and not t.tgisinternal
  ) then
    create trigger support_ticket_comments_audit after insert or update or delete on public.support_ticket_comments for each row execute function public.write_support_ticket_audit_log();
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'support_ticket_comments' and policyname = 'support_ticket_comments_select'
  ) then
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'support_ticket_comments' and policyname = 'support_ticket_comments_insert'
  ) then
...
-- =============================================================================
-- 215 — ticket_platform_foundation_sprint6_9_2
-- =============================================================================
...
-- =============================================================================
-- 216 — ticket_platform_metrics_aggregation_sprint6_9_2b
-- =============================================================================
...
-- =============================================================================
-- 217 — human_handoff_platform_sprint6_10
-- =============================================================================
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'handoff_queues' and policyname = 'handoff_queues_tenant'
  ) then
    create policy handoff_queues_tenant on public.handoff_queues for all using (public.company_has_permission(company_id, 'handoff.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'handoff_queue_members' and policyname = 'handoff_queue_members_tenant'
  ) then
    create policy handoff_queue_members_tenant on public.handoff_queue_members for all using (public.company_has_permission(company_id, 'handoff.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'handoff_conversation_ownership' and policyname = 'handoff_ownership_tenant'
  ) then
    create policy handoff_ownership_tenant on public.handoff_conversation_ownership for all using (public.company_has_permission(company_id, 'handoff.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'handoff_ownership_history' and policyname = 'handoff_ownership_history_tenant'
  ) then
    create policy handoff_ownership_history_tenant on public.handoff_ownership_history for all using (public.company_has_permission(company_id, 'handoff.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'handoff_requests' and policyname = 'handoff_requests_tenant'
  ) then
    create policy handoff_requests_tenant on public.handoff_requests for all using (public.company_has_permission(company_id, 'handoff.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'handoff_context_snapshots' and policyname = 'handoff_context_snapshots_tenant'
  ) then
    create policy handoff_context_snapshots_tenant on public.handoff_context_snapshots for all using (public.company_has_permission(company_id, 'handoff.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'agent_presence' and policyname = 'agent_presence_tenant'
  ) then
    create policy agent_presence_tenant on public.agent_presence for all using (public.company_has_permission(company_id, 'handoff.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'handoff_escalation_rules' and policyname = 'handoff_escalation_rules_tenant'
  ) then
    create policy handoff_escalation_rules_tenant on public.handoff_escalation_rules for all using (public.company_has_permission(company_id, 'handoff.view'));
  end if;
end $$;
...
-- =============================================================================
-- 218 — lead_platform_foundation_sprint6_11b
-- =============================================================================
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_pipelines' and policyname = 'lead_pipelines_tenant'
  ) then
    create policy lead_pipelines_tenant on public.lead_pipelines for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_stages' and policyname = 'lead_stages_tenant'
  ) then
    create policy lead_stages_tenant on public.lead_stages for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_sources' and policyname = 'lead_sources_tenant'
  ) then
    create policy lead_sources_tenant on public.lead_sources for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'leads' and policyname = 'leads_tenant'
  ) then
    create policy leads_tenant on public.leads for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_assignments' and policyname = 'lead_assignments_tenant'
  ) then
    create policy lead_assignments_tenant on public.lead_assignments for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_scores' and policyname = 'lead_scores_tenant'
  ) then
    create policy lead_scores_tenant on public.lead_scores for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_tags' and policyname = 'lead_tags_tenant'
  ) then
    create policy lead_tags_tenant on public.lead_tags for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_notes' and policyname = 'lead_notes_tenant'
  ) then
    create policy lead_notes_tenant on public.lead_notes for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_activities' and policyname = 'lead_activities_tenant'
  ) then
    create policy lead_activities_tenant on public.lead_activities for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_history' and policyname = 'lead_history_tenant'
  ) then
    create policy lead_history_tenant on public.lead_history for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_conversion_history' and policyname = 'lead_conversion_history_tenant'
  ) then
    create policy lead_conversion_history_tenant on public.lead_conversion_history for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_custom_fields' and policyname = 'lead_custom_fields_tenant'
  ) then
    create policy lead_custom_fields_tenant on public.lead_custom_fields for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'lead_import_batches' and policyname = 'lead_import_batches_tenant'
  ) then
    create policy lead_import_batches_tenant on public.lead_import_batches for all using (public.company_has_permission(company_id, 'leads.view'));
  end if;
end $$;
...
-- =============================================================================
-- 219 — business_os_identity_integration_sprint6_11d
-- =============================================================================
...
-- =============================================================================
-- 220 — universal_entity_foundation_ga1_1
-- =============================================================================
...
begin
  if not exists (
    ...
    join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'entity_contacts' and tr.tgname = 'trg_entity_contacts_updated_at' and not tr.tgisinternal
  ) then
    create trigger trg_entity_contacts_updated_at before update on public.entity_contacts for each row execute function public.trg_entity_set_updated_at();
  end if;
end $$;
...
begin
  if not exists (
    ...
    join pg_class c on c.oid = tr.tgrelid
    join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'entity_files' and tr.tgname = 'trg_entity_files_updated_at' and not tr.tgisinternal
  ) then
    create trigger trg_entity_files_updated_at before update on public.entity_files for each row execute function public.trg_entity_set_updated_at();
  end if;
end $$;
...
begin
  if not exists (
    ...
    join pg_class c on c.oid = tr.tgrelid
    join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'entity_tags' and tr.tgname = 'trg_entity_tags_updated_at' and not tr.tgisinternal
  ) then
    create trigger trg_entity_tags_updated_at before update on public.entity_tags for each row execute function public.trg_entity_set_updated_at();
  end if;
end $$;
...
begin
  if not exists (
    ...
    join pg_class c on c.oid = tr.tgrelid
    join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'entity_custom_fields' and tr.tgname = 'trg_entity_custom_fields_updated_at' and not tr.tgisinternal
  ) then
    create trigger trg_entity_custom_fields_updated_at before update on public.entity_custom_fields for each row execute function public.trg_entity_set_updated_at();
  end if;
end $$;
...
begin
  if not exists (
    ...
    join pg_class c on c.oid = tr.tgrelid
    join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'entity_custom_field_values' and tr.tgname = 'trg_entity_custom_field_values_updated_at' and not tr.tgisinternal
  ) then
    create trigger trg_entity_custom_field_values_updated_at before update on public.entity_custom_field_values for each row execute function public.trg_entity_set_updated_at();
  end if;
end $$;
...
begin
  if not exists (
    ...
    join pg_class c on c.oid = tr.tgrelid
    join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'entity_activities' and tr.tgname = 'trg_entity_activities_updated_at' and not tr.tgisinternal
  ) then
    create trigger trg_entity_activities_updated_at before update on public.entity_activities for each row execute function public.trg_entity_set_updated_at();
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_contacts' and policyname = 'entity_contacts_select'
  ) then
    create policy entity_contacts_select on public.entity_contacts for select to authenticated using (
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_contacts' and policyname = 'entity_contacts_insert'
  ) then
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_contacts' and policyname = 'entity_contacts_update'
  ) then
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_contacts' and policyname = 'entity_contacts_delete'
  ) then
    create policy entity_contacts_delete on public.entity_contacts for delete to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_files' and policyname = 'entity_files_select'
  ) then
    create policy entity_files_select on public.entity_files for select to authenticated using (
      public.is_super_admin() or (tenant_id = public.current_company_id() and deleted_at is null)
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_files' and policyname = 'entity_files_insert'
  ) then
    create policy entity_files_insert on public.entity_files for insert to authenticated with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_files' and policyname = 'entity_files_update'
  ) then
    create policy entity_files_update on public.entity_files for update to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    ) with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_files' and policyname = 'entity_files_delete'
  ) then
    create policy entity_files_delete on public.entity_files for delete to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_tags' and policyname = 'entity_tags_select'
  ) then
    create policy entity_tags_select on public.entity_tags for select to authenticated using (
      public.is_super_admin() or (tenant_id = public.current_company_id() and deleted_at is null)
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_tags' and policyname = 'entity_tags_insert'
  ) then
    create policy entity_tags_insert on public.entity_tags for insert to authenticated with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_tags' and policyname = 'entity_tags_update'
  ) then
    create policy entity_tags_update on public.entity_tags for update to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    ) with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_tags' and policyname = 'entity_tags_delete'
  ) then
    create policy entity_tags_delete on public.entity_tags for delete to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_tag_assignments' and policyname = 'entity_tag_assignments_select'
  ) then
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_tag_assignments' and policyname = 'entity_tag_assignments_insert'
  ) then
    create policy entity_tag_assignments_insert on public.entity_tag_assignments for insert to authenticated with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_tag_assignments' and policyname = 'entity_tag_assignments_update'
  ) then
    create policy entity_tag_assignments_update on public.entity_tag_assignments for update to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    ) with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_tag_assignments' and policyname = 'entity_tag_assignments_delete'
  ) then
    create policy entity_tag_assignments_delete on public.entity_tag_assignments for delete to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_custom_fields' and policyname = 'entity_custom_fields_select'
  ) then
    create policy entity_custom_fields_select on public.entity_custom_fields for select to authenticated using (
      public.is_super_admin() or (tenant_id = public.current_company_id() and deleted_at is null)
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_custom_fields' and policyname = 'entity_custom_fields_insert'
  ) then
    create policy entity_custom_fields_insert on public.entity_custom_fields for insert to authenticated with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_custom_fields' and policyname = 'entity_custom_fields_update'
  ) then
    create policy entity_custom_fields_update on public.entity_custom_fields for update to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    ) with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_custom_fields' and policyname = 'entity_custom_fields_delete'
  ) then
    create policy entity_custom_fields_delete on public.entity_custom_fields for delete to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_custom_field_values' and policyname = 'entity_custom_field_values_select'
  ) then
    create policy entity_custom_field_values_select on public.entity_custom_field_values for select to authenticated using (
      public.is_super_admin() or (tenant_id = public.current_company_id() and deleted_at is null)
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_custom_field_values' and policyname = 'entity_custom_field_values_insert'
  ) then
    create policy entity_custom_field_values_insert on public.entity_custom_field_values for insert to authenticated with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_custom_field_values' and policyname = 'entity_custom_field_values_update'
  ) then
    create policy entity_custom_field_values_update on public.entity_custom_field_values for update to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    ) with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_custom_field_values' and policyname = 'entity_custom_field_values_delete'
  ) then
    create policy entity_custom_field_values_delete on public.entity_custom_field_values for delete to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_activities' and policyname = 'entity_activities_select'
  ) then
    create policy entity_activities_select on public.entity_activities for select to authenticated using (
      public.is_super_admin() or (tenant_id = public.current_company_id() and deleted_at is null)
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_activities' and policyname = 'entity_activities_insert'
  ) then
    create policy entity_activities_insert on public.entity_activities for insert to authenticated with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_activities' and policyname = 'entity_activities_update'
  ) then
    create policy entity_activities_update on public.entity_activities for update to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    ) with check (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'entity_activities' and policyname = 'entity_activities_delete'
  ) then
    create policy entity_activities_delete on public.entity_activities for delete to authenticated using (
      public.is_super_admin() or tenant_id = public.current_company_id()
    );
  end if;
end $$;
...
-- =============================================================================
-- 222 — tasks_and_operations_workspace_ga1_3
-- =============================================================================
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'operations_workspace_config' and policyname = 'operations_workspace_config_select'
  ) then
    create policy operations_workspace_config_select on public.operations_workspace_config for select using (
        company_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'operations_workspace_config' and policyname = 'operations_workspace_config_insert'
  ) then
    create policy operations_workspace_config_insert on public.operations_workspace_config for insert with check (
        company_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'operations_workspace_config' and policyname = 'operations_workspace_config_update'
  ) then
    create policy operations_workspace_config_update on public.operations_workspace_config for update using (
        company_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
-- =============================================================================
-- 223 — ga1_4_enterprise_configuration_platform
-- =============================================================================
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_configurations' and policyname = 'platform_configurations_select'
  ) then
    create policy platform_configurations_select on public.platform_configurations for select using (
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_configurations' and policyname = 'platform_configurations_insert'
  ) then
    create policy platform_configurations_insert on public.platform_configurations for insert with check (
        tenant_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_configurations' and policyname = 'platform_configurations_update'
  ) then
    create policy platform_configurations_update on public.platform_configurations for update using (
        tenant_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_configuration_versions' and policyname = 'platform_configuration_versions_select'
  ) then
    create policy platform_configuration_versions_select on public.platform_configuration_versions for select using (
        tenant_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_configuration_versions' and policyname = 'platform_configuration_versions_insert'
  ) then
    create policy platform_configuration_versions_insert on public.platform_configuration_versions for insert with check (
        tenant_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
-- =============================================================================
-- 224 — feature_flags_and_licensing_ga1_4
-- =============================================================================
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_feature_flags' and policyname = 'platform_feature_flags_select'
  ) then
    create policy platform_feature_flags_select on public.platform_feature_flags for select using (auth.role() = 'authenticated');
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_feature_flags' and policyname = 'platform_feature_flags_write'
  ) then
    create policy platform_feature_flags_write on public.platform_feature_flags for all using (
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_feature_flag_versions' and policyname = 'platform_feature_flag_versions_select'
  ) then
    create policy platform_feature_flag_versions_select on public.platform_feature_flag_versions for select using (
        tenant_id is null
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_plan_entitlements' and policyname = 'platform_plan_entitlements_select'
  ) then
    create policy platform_plan_entitlements_select on public.platform_plan_entitlements for select using (auth.role() = 'authenticated');
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_company_licenses' and policyname = 'platform_company_licenses_select'
  ) then
    create policy platform_company_licenses_select on public.platform_company_licenses for select using (
        tenant_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_company_licenses' and policyname = 'platform_company_licenses_write'
  ) then
    create policy platform_company_licenses_write on public.platform_company_licenses for update using (
        exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
end $$;
...
-- =============================================================================
-- 225 — ga1_5_enterprise_reactive_platform
-- =============================================================================
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_event_audit' and policyname = 'platform_event_audit_select'
  ) then
    create policy platform_event_audit_select on public.platform_event_audit for select using (
        tenant_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_event_timeline' and policyname = 'platform_event_timeline_select'
  ) then
    create policy platform_event_timeline_select on public.platform_event_timeline for select using (
        tenant_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_event_dlq' and policyname = 'platform_event_dlq_select'
  ) then
    create policy platform_event_dlq_select on public.platform_event_dlq for select using (
        tenant_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_reactive_signals' and policyname = 'platform_reactive_signals_select'
  ) then
    create policy platform_reactive_signals_select on public.platform_reactive_signals for select using (
        tenant_id in (select company_id from public.profiles where id = auth.uid())
        or exists (select 1 from public.profiles where id = auth.uid() and is_super_admin = true)
      );
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_event_audit' and policyname = 'platform_event_audit_insert'
  ) then
    create policy platform_event_audit_insert on public.platform_event_audit for insert with check (auth.role() in ('authenticated', 'service_role'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_event_timeline' and policyname = 'platform_event_timeline_insert'
  ) then
    create policy platform_event_timeline_insert on public.platform_event_timeline for insert with check (auth.role() in ('authenticated', 'service_role'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_event_dlq' and policyname = 'platform_event_dlq_insert'
  ) then
    create policy platform_event_dlq_insert on public.platform_event_dlq for insert with check (auth.role() in ('authenticated', 'service_role'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_event_subscriber_telemetry' and policyname = 'platform_event_subscriber_telemetry_insert'
  ) then
    create policy platform_event_subscriber_telemetry_insert on public.platform_event_subscriber_telemetry for insert with check (auth.role() in ('authenticated', 'service_role'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_event_correlations' and policyname = 'platform_event_correlations_insert'
  ) then
    create policy platform_event_correlations_insert on public.platform_event_correlations for insert with check (auth.role() in ('authenticated', 'service_role'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_event_idempotency' and policyname = 'platform_event_idempotency_all'
  ) then
    create policy platform_event_idempotency_all on public.platform_event_idempotency for all using (auth.role() in ('authenticated', 'service_role'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_reactive_signals' and policyname = 'platform_reactive_signals_insert'
  ) then
    create policy platform_reactive_signals_insert on public.platform_reactive_signals for insert with check (auth.role() in ('authenticated', 'service_role'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'platform_event_dlq' and policyname = 'platform_event_dlq_update'
  ) then
    create policy platform_event_dlq_update on public.platform_event_dlq for update using (auth.role() in ('authenticated', 'service_role'));
  end if;
end $$;
...
-- =============================================================================
-- 226 — omnichannel_realtime_publication
-- =============================================================================
...
-- =============================================================================
-- 227 — appointment_platform_promotion_sprint6_12
-- =============================================================================
