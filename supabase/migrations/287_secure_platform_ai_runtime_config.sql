-- ============================================================
-- 287 — Secure platform_resolve_ai_runtime_config
--
-- Browser must never receive decrypted provider API keys.
-- Login-app now proxies AI chat/embeddings via api-server
-- (service_role resolves credentials).
--
-- This migration:
--   1) Hardens the RPC: only service_role may decrypt/return apiKey
--   2) REVOKE authenticated EXECUTE from the exact signature
--
-- Preserves service_role EXECUTE.
-- Does not touch unrelated functions / RLS / search_path.
-- ============================================================
...
begin
  ...
  -- Credential resolution is backend-only (service_role).
  if auth.role() is distinct from 'service_role' then
    raise exception 'service_role required for platform AI runtime credential resolution';
  end if;
...
revoke execute on function public.platform_resolve_ai_runtime_config(uuid, text, text) from authenticated;
revoke execute on function public.platform_resolve_ai_runtime_config(uuid, text, text) from anon;
revoke execute on function public.platform_resolve_ai_runtime_config(uuid, text, text) from public;

comment on function public.platform_resolve_ai_runtime_config(uuid, text, text) is
  'Backend-only platform AI credential resolver (service_role). Decrypts provider API keys for server-side execution. Must never be callable from browser/authenticated clients.';
