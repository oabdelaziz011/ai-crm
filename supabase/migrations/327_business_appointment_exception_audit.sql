-- Additive audit enrichment for Business Appointment Exception / Apology.
-- Preserves all existing write_audit_log() entity branches; adds exception + booking cancel metadata.
-- Does not modify existing booking rows or notification tables.

-- ---------------------------------------------------------------------------
-- 1) Booking → exception link (nullable; set only by apology cancel path)
-- ---------------------------------------------------------------------------
alter table public.scheduling_bookings
  add column if not exists business_exception_id uuid
    references public.business_appointment_exceptions(id) on delete set null;

alter table public.scheduling_bookings
  add column if not exists business_exception_item_id uuid
    references public.business_appointment_exception_items(id) on delete set null;

create index if not exists idx_scheduling_bookings_company_business_exception
  on public.scheduling_bookings(company_id, business_exception_id)
  where business_exception_id is not null;

create index if not exists idx_scheduling_bookings_business_exception_item
  on public.scheduling_bookings(business_exception_item_id)
  where business_exception_item_id is not null;

comment on column public.scheduling_bookings.business_exception_id is
  'Set when cancelled via Business Appointment Exception; NULL for normal cancellations.';
comment on column public.scheduling_bookings.business_exception_item_id is
  'Per-booking exception item that drove cancellation; NULL for normal cancellations.';

-- ---------------------------------------------------------------------------
-- 2) write_audit_log — preserve prior branches; enrich exception + scheduling bookings
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) Audit trigger on exception items
-- ---------------------------------------------------------------------------
drop trigger if exists trg_audit_business_appointment_exception_items
  on public.business_appointment_exception_items;
create trigger trg_audit_business_appointment_exception_items
  after insert or update or delete on public.business_appointment_exception_items
  for each row execute procedure public.write_audit_log();

comment on trigger trg_audit_business_appointment_exception_items
  on public.business_appointment_exception_items is
  'Audit INSERT/UPDATE/DELETE for business appointment exception items (cancel + notify lifecycle).';
