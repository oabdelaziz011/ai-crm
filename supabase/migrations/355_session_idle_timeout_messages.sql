-- Session idle timeout: warning (at 50%) + ended message templates on AI Assistant settings.

alter table public.ai_assistant_settings
  add column if not exists session_idle_warning_message text not null default
    'تنبيه: جلسة المحادثة ستنتهي قريباً. يرجى الرد سريعاً لاستكمال المحادثة.';

alter table public.ai_assistant_settings
  add column if not exists session_ended_message text not null default
    'انتهت جلسة المحادثة بسبب عدم الرد. يمكنك إرسال رسالة جديدة لبدء جلسة جديدة.';

alter table public.ai_assistant_settings
  drop constraint if exists ai_assistant_settings_session_idle_warning_message_length_check;

alter table public.ai_assistant_settings
  add constraint ai_assistant_settings_session_idle_warning_message_length_check
  check (char_length(session_idle_warning_message) <= 1000);

alter table public.ai_assistant_settings
  drop constraint if exists ai_assistant_settings_session_ended_message_length_check;

alter table public.ai_assistant_settings
  add constraint ai_assistant_settings_session_ended_message_length_check
  check (char_length(session_ended_message) <= 1000);
